...
import torchvision
from utils.general_utils import safe_state, vis_depth, save_mask
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, masked_psnr=False):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        depth = renders["depth"].detach().cpu().numpy().squeeze()
        depth = vis_depth(depth)
        cv2.imwrite(os.path.join(render_depth_path, view.image_name + "_depth.png"), depth)
        if masked_psnr:
            save_mask(os.path.join(masks_path, view.image_name + ".png"), view.fg_mask)

def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool):
    with torch.no_grad():
        gaussians = GaussianModel(dataset.sh_degree)
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False, stage=pipeline.stage)
